...
export interface AIReviewResult {
    ...
    confidence: number;
    explanation: string;
    aiGeneratedConfidence: number;  // 0-1 — how likely the code was AI-generated
...
JOB 2 — AI-GENERATED CODE DETECTION (BE STRICT):
...
- Code that looks "cleaned up" from AI output — comments removed but robotic structure remains
...
Human-written code in Discord challenges is usually rough, minimal, sometimes messy — that's NORMAL.
...
export async function processReviewJob(job: ReviewJob): Promise<void> {
    ...
    try {
        ...
        // 2.5 AI-generated code check — reject if flagged
        if (result.aiGeneratedConfidence >= 0.75) {
            ...
            const rejectMsg = [
                ...
                "",
                "Write your own code — you don't get a job copying from ChatGPT.",
...
async function sendAuditLog(
    ...
    const statusEmoji = aiDetected ? "🤖" : status === "CORRECT" ? "✅" : status === "PARTIAL" ? "🟡" : "❌";
    ...
    const logLines = [
        ...
        `├─ Status: ${statusEmoji} ${aiDetected ? "AI-GENERATED" : status}`,
        `├─ Confidence: ${(result.confidence * 100).toFixed(0)}%`,
    ];

    if (aiDetected) {
        logLines.push(`├─ AI Detection: ${(result.aiGeneratedConfidence * 100).toFixed(0)}% — ${result.aiGeneratedReason.slice(0, 150)}`);
    }
